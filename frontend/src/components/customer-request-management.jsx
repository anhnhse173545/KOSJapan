...
const BookingRow = ({ booking, onTripStatusUpdate, onViewBooking, onCreateOrEditTrip, onManageTripDestinations }) => {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'Pending Quote': return 'bg-yellow-200 text-yellow-800'
      case 'Approved Quote': return 'bg-green-200 text-green-800'
      case 'Pending': return 'bg-orange-200 text-orange-800'
      case 'Approved': return 'bg-emerald-200 text-emerald-800'
      case 'Completed': return 'bg-indigo-200 text-indigo-800'
      case 'On-going': return 'bg-purple-200 text-purple-800'
      case 'Redo': return 'bg-red-200 text-red-800'
    ...
    (<TableRow className="hover:bg-muted/50 transition-colors">
      ...
      <TableCell>
        <div className="flex space-x-2">
          ...
          <Button variant="outline" onClick={() => onCreateOrEditTrip(booking)}>
            {booking.trip ? 'Edit Trip' : 'Create Trip'}
          </Button>
          <Button variant="outline" onClick={() => onManageTripDestinations(booking)}>
    ...
    (<Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[600px]">
        ...
        <Tabs value={step} onValueChange={setStep}>
          <TabsList>
            <TabsTrigger value="trip" disabled={showDestinations}>Trip Details</TabsTrigger>
            <TabsTrigger value="destinations" disabled={!booking.trip || !showDestinations}>Destinations</TabsTrigger>
          </TabsList>
...
export function CustomerRequestManagementComponent() {
  ...
  const handleViewBooking = async (bookingId) => {
    try {
      ...
      toast(
        { title: "Error", description: "Failed to fetch booking details", variant: "destructive" }
      )
    }
  }
  ...
  const handleCreateOrEditTrip = (booking) => {
    if (booking) {
      ...
      toast(
        { title: "Error", description: "Invalid booking selected", variant: "destructive" }
      )
    }
  }
  ...
  const handleTripCreated = (updatedTrip) => {
    ...
    toast({ title: "Success", description: "Trip created/updated successfully" })
  }
