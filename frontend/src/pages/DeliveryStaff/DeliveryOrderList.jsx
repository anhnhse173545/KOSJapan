...
export default function DeliveryOrderListComponent() {
    ...
    const fetchBookings = async () => {
      ...
      try {
        const [staffBookingsResponse, orderPrepareResponse] = await Promise.all(
          [
            fetch(
              `http://localhost:8080/api/booking/delivery-staff/${staff.id}`
            ),
        ...
        const combinedBookings = staffBookings.filter((booking) =>
          orderPrepareBookings.some(
            (prepareBooking) => prepareBooking.id === booking.id
          )
        );
  ...
  const handleUpdateBookingStatus = async (bookingId, newStatus) => {
    try {
      const response = await fetch(
        `http://localhost:8080/api/booking/update/${bookingId}`,
        {
          method: "PUT",
          headers: {
  ...
  const handleUpdateFishOrderStatus = async (
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8080/fish-order/${bookingId}/${farmId}/update`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <div className="container mx-auto py-10">
      <h1 className="text-2xl font-bold mb-5">
        Order Prepare List for {staff?.name}
      </h1>
      {bookings.length === 0 ? (
      ...
      {showTrackingCard && selectedOrder && (
        ...
        >
          <OrderTrackingCard
            order={selectedOrder}
            onClose={() => setShowTrackingCard(false)}
          />
        </div>
      )}
    </div>
...
// const paymentStatuses = ['Deposited', 'Paid Full']
...
//           delivery_address: booking.customer.address,
//           arrived_date: new Date().toISOString(),
//           paymentStatus: booking.fishOrders.find(o => o.id === orderId)?.paymentStatus || 'Deposited'
//         }),
//       })
...
//   const handleUpdatePaymentStatus = async (bookingId, farmId, orderId, newPaymentStatus) => {
//     try {
//       const booking = bookings.find(b => b.id === bookingId)
//       if (!booking) throw new Error('Booking not found')
...
//                             <DropdownMenuTrigger asChild>
//                               <Button variant="outline" size="sm">
//                                 {order.paymentStatus || 'Select Payment Status'} <ChevronDown className="ml-2 h-4 w-4" />
//                               </Button>
//                             </DropdownMenuTrigger>
//                             <DropdownMenuContent>
...
//                                 <DropdownMenuItem
//                                   key={status}
//                                   onSelect={() => handleUpdatePaymentStatus(booking.id, order.farmId, order.id, status)}
//                                 >
//                                   {status}
//                                 </DropdownMenuItem>
//                               ))}
