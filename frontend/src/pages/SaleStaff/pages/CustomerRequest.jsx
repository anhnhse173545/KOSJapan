        ...
        {booking.trip ? (
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="outline"
                className={getStatusColor(booking.trip.status)}
              >
            ...
            <DropdownMenuContent>
              {tripStatusOptions.map((status) => (
                <DropdownMenuItem
                  key={status}
                  onClick={() => onTripStatusUpdate(booking.id, status)}
                >
                  {status}
                </DropdownMenuItem>
              ))}
      ...
      <TableCell>
        <div className="flex space-x-2">
          <Button
            ...
            style={{ color: "#007bff", borderColor: "#007bff" }} // Blue for "View Booking"
          >
            View Booking
          </Button>
          <Button
            ...
            style={{ color: "#28a745", borderColor: "#28a745" }} // Green for "Edit/Create Trip"
          >
          ...
          <Button
            ...
            style={{ color: "#fd7e14", borderColor: "#fd7e14" }} // Orange for "Add/Edit Destinations"
          >
        ...
        <ScrollArea className="mt-4 max-h-[60vh]">
          <div className="space-y-4">
            ...
            <div>
              ...
              <p>
                Created At: {new Date(bookingDetails.createAt).toLocaleString()}
              </p>
            ...
            {bookingDetails.trip && (
              <div>
                ...
                <p>
                  Start Date:{" "}
                  {new Date(bookingDetails.trip.startDate).toLocaleDateString()}
                </p>
                <p>
                  End Date:{" "}
                  {new Date(bookingDetails.trip.endDate).toLocaleDateString()}
                </p>
              </div>
            )}
          </div>
...
const CreateOrEditTripModal = ({
  ...
  const fetchTripDestinations = async (tripId) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/trip-destination/${tripId}/list`
      );
  ...
  //       price: parseFloat(tripDetails.price),
  //     };
  ...
  const handleAddDestination = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/trip-destination/${booking.trip.id}/create`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      <DialogContent className="sm:max-w-[600px]">
        ...
        <Tabs value={step} onValueChange={setStep}>
          <TabsList>
            <TabsTrigger
              value="trip"
              disabled={showDestinations}
              className="text-black" // Ensures the text is always black and visible
            >
              Trip Details
            </TabsTrigger>
            <TabsTrigger
              value="destinations"
              disabled={!booking.trip || !showDestinations}
              className="text-black" // Ensures the text is always black and visible
            >
              Destinations
            </TabsTrigger>
                ...
                <form onSubmit={handleAddDestination} className="space-y-4">
                  <div className="flex flex-col space-y-1.5">
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        setNewDestination({ ...newDestination, farmId: value })
                      }
                    >
                      <SelectTrigger className="text-black">
                        {" "}
                        {/* Ensures text is black and visible */}
...
export default function CustomerRequestManagement() {
  ...
  const [isManageDestinationsModalOpen, setIsManageDestinationsModalOpen] =
    useState(false);
  const navigate = useNavigate();
  ...
  const handleTripStatusUpdate = async (bookingId, newStatus) => {
    try {
      ...
      setBookings((prevBookings) =>
        prevBookings.map((b) =>
          b.id === bookingId
            ? { ...b, trip: { ...b.trip, status: updatedTrip.status } }
            : b
        )
      );
      toast({
